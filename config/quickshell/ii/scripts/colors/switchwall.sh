    ...
    if [[ "$color_flag" == "1" ]]; then
        ...
        if is_video "$imgpath"; then
            ...
            if [ -f "$thumbnail" ]; then
                ...
                # create_restore_script "$video_path"  # Disabled: no longer create restore script
            else
                echo "Cannot create image to colorgen"
                # remove_restore  # Disabled
                exit 1
            fi
        else
            ...
            set_wallpaper_path "$imgpath"
            # remove_restore  # Disabled: no longer create/remove restore script
        fi
    fi
