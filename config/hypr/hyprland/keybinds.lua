-- =============================================================================
-- Hyprland Keybinds (Lua) — tương đương hyprland/keybinds.conf
--
-- cheatsheet (quickshell): trong conf, dòng kết thúc bằng `# [hidden]` không hiện
-- trên cheatsheet; trong Lua các bind tương ứng ghi `-- [hidden]` ở cuối dòng.
--
-- Cấu trúc conf: `#!` / `##!` / `###` là tiêu đề; ở đây dùng `--!` cho các khối lớn.
-- Một số chỗ gọn hơn conf (vòng lặp số workspace, hướng focus, v.v.).
-- =============================================================================
...
-- ### Overview & Launcher
-- Các bind overview phải ở trên cùng (keybinds.conf: "These absolutely need to be on top")
...
bind("Super", "Super_L", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || pkill fuzzel || fuzzel") -- [hidden]
bind("Super", "Super_R", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || pkill fuzzel || fuzzel") -- [hidden]
...
-- bind("Super", "Tab", "global", "quickshell:overviewWorkspacesToggle") -- [hidden] alt overview (conf)
bind("Super", "Tab", "focuscurrentorlast") -- [hidden] Switch between 2 most recent apps
-- bind("Alt", "Space", "exec", "vicinae toggle") -- conf: vicinae (tắt)
...
-- bind("Shift+Super+Alt", "Slash", "exec", "qs -p ~/.config/quickshell/" .. QSP .. "/welcome.qml") -- [hidden] welcome (conf; cần đường dẫn đúng profile)
...
local bright_scripts = {
  up   = "qs -c " .. QSP .. " ipc call brightness increment || brightnessctl s 5%+",
  down = "qs -c " .. QSP .. " ipc call brightness decrement || brightnessctl s 5%-",
}
...
--! =============================================================================
--! UTILITIES (keybinds.conf: #! ##! Utilities)
--! =============================================================================

-- ### Screenshots & Visual Capture
-- Fallback clipboard/emoji khi qs không chạy — xem keybinds.conf (bindd Super V / Period với exec fuzzel…)
bind("Super+Shift", "S", "global", "quickshell:regionScreenshot")
bind("Super+Shift", "S", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || pidof slurp || ~/.config/hypr/hyprland/scripts/screenshot.sh --region --clipboard") -- [hidden]

bind("Super+Shift", "A", "global", "quickshell:regionSearch")
bind("Super+Shift", "A", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || pidof slurp || ~/.config/hypr/hyprland/scripts/snip_to_search.sh") -- [hidden]
...
bind("Super+Shift", "X", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || pidof slurp || grim -g \"$(slurp $SLURP_ARGS)\" \"/tmp/ocr_image.png\" && tesseract \"/tmp/ocr_image.png\" stdout -l $(tesseract --list-langs | awk 'NR>1{print $1}' | tr '\\\\n' '+' | sed 's/\\\\+$/\\\\n/') | wl-copy && rm \"/tmp/ocr_image.png\"") -- [hidden]
bind("Super+Shift", "T", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || pidof slurp || grim -g \"$(slurp $SLURP_ARGS)\" \"/tmp/ocr_image.png\" && tesseract \"/tmp/ocr_image.png\" stdout -l $(tesseract --list-langs | awk 'NR>1{print $1}' | tr '\\\\n' '+' | sed 's/\\\\+$/\\\\n/') | wl-copy && rm \"/tmp/ocr_image.png\"") -- [hidden]
...
bindl("Super+Shift", "R", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || " .. QSD .. "/scripts/videos/record.sh") -- [hidden]
bindl("Super+Alt", "R", "global", "quickshell:regionRecord") -- [hidden]
bindl("Super+Alt", "R", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || " .. QSD .. "/scripts/videos/record.sh") -- [hidden]
bindl("Ctrl+Alt", "R", "exec", QSD .. "/scripts/videos/record.sh --fullscreen") -- [hidden]
bindl("Super+Shift+Alt", "R", "exec", QSD .. "/scripts/videos/record.sh --fullscreen --sound")

-- ### AI Helpers
bindd("Super+Shift+Alt", "mouse:273", "Generate AI summary for selected text", "exec", "~/.config/hypr/hyprland/scripts/ai/primary-buffer-query.sh") -- [hidden]

-- ### Panels & helpers (conf: Wallpaper / Appearance / Panels — một phần ở đây, một phần dưới)
...
bind("Ctrl+Alt", "Delete", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || pkill wlogout || wlogout -p layer-shell") -- [hidden]
...
bind("Ctrl+Super", "T", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || " .. QSD .. "/scripts/colors/switchwall.sh") -- [hidden]
bind("Super+Alt", "W", "global", "quickshell:panelFamilyCycle")

-- Restart widgets
bind("Ctrl+Super", "R", "exec", "killall ags agsv1 gjs ydotool qs quickshell; qs -c " .. QSP .. " &")
...
-- #/# cheatsheet: Super + arrows → movefocus (conf dùng Left/Right/Up/Down)
...
-- #/# cheatsheet: Super+Shift + arrows → swapwindow
for key, dir in pairs(dirs) do
  bind("Super+Shift", key, "swapwindow", dir) -- [hidden]
end

-- bind("Alt", "F4", …) — Close (Windows); Super+Q — Close (conf)
...
-- #/# cheatsheet: Super ; ' → layoutmsg splitratio (Hyprland 0.54+ vẫn dùng layoutmsg splitratio trong conf)
binde("Super", "Semicolon", "layoutmsg", "splitratio", "-0.1") -- [hidden] dwindle: was splitratio dispatcher (removed 0.54+)
...
-- #/# cheatsheet: Super+Alt + số → movetoworkspacesilent
-- Dùng keycode thô vì layout bàn phím khác nhau; kiểm tra bằng `wev` (keybinds.conf)
...
-- Hyprscrolling (conf, tắt): Super+Shift + mouse_down/up → layoutmsg move ±col
...
-- #/# cheatsheet: Super + số → workspace; keycode + `wev` như trên
-- Main number row
for i = 1, 10 do
...
--! =============================================================================
-- Phím tắt workspace 11–20 (màn hình phụ); Ctrl+Super tránh trùng Super+Shift (conf)
...
-- ### Overview & Paging
-- hyprexpo: dùng exec để tránh lỗi parse dispatcher nếu plugin load muộn (conf)
...
--! =============================================================================
--! SESSION & SCREEN (conf tách ##! Session / ##! Screen)
--! =============================================================================

-- ##! Session
-- bindl("", "switch:on:Lid Switch", "exec", "systemctl suspend || loginctl suspend") -- [hidden] conf: suspend khi gập nắp (tắt)
...
-- ##! Screen
-- Zoom
binde("Super", "Minus", "exec", "qs -c " .. QSP .. " ipc call zoom zoomOut")
binde("Super", "Equal",  "exec", "qs -c " .. QSP .. " ipc call zoom zoomIn")
binde("Super", "Minus", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || ~/.config/hypr/hyprland/scripts/zoom.sh decrease 0.1") -- [hidden]
binde("Super", "Equal",  "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || ~/.config/hypr/hyprland/scripts/zoom.sh increase 0.1") -- [hidden]
binde("Super", "code:82", "exec", "qs -c " .. QSP .. " ipc call zoom zoomOut") -- [hidden] keypad 0
binde("Super", "code:86", "exec", "qs -c " .. QSP .. " ipc call zoom zoomIn")   -- [hidden] keypad +
binde("Super", "code:82", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || ~/.config/hypr/hyprland/scripts/zoom.sh decrease 0.1") -- [hidden]
binde("Super", "code:86", "exec", "qs -c " .. QSP .. " ipc call TEST_ALIVE || ~/.config/hypr/hyprland/scripts/zoom.sh increase 0.1") -- [hidden]
...
-- Terminal / file manager / browser / editor (conf có mô tả ngắn từng dòng)
...
bind("Super", "I", "exec", "XDG_CURRENT_DESKTOP=gnome ~/.config/hypr/hyprland/scripts/launch_first_available.sh \"qs -p " .. QSD .. "/settings.qml\" \"systemsettings\" \"gnome-control-center\" \"better-control\"")
...
--! =============================================================================
--! TESTING (conf: `# Testing` trước ##! Session — thứ tự khác một chút)
--! =============================================================================
...
--! ##! Custom
-- Open shell config / Open Hyprland keybinds (conf)
bind("Ctrl+Super", "Backslash", "resizeactive", "exact 640 480") -- [hidden] Cursed stuff: Make window not amogus large
...
-- ##! Hyprscrolling (tắt trong keybinds.conf — bật layout scrolling rồi mở comment)
-- bind("Super", "Y", "layoutmsg", "move -col")
-- bind("Super", "U", "layoutmsg", "move +col")
-- bind("Super+Shift", "Y", "layoutmsg", "movewindowto l")
-- bind("Super+Shift", "U", "layoutmsg", "movewindowto r")
-- bind("Super+Shift", "I", "layoutmsg", "movewindowto u")
-- bind("Super+Shift", "O", "layoutmsg", "movewindowto d")
-- bind("Super+Alt", "Y", "layoutmsg", "colresize -0.1")
-- bind("Super+Alt", "U", "layoutmsg", "colresize +0.1")
-- bind("Super+Alt", "P", "layoutmsg", "promote")
-- bind("Super+Alt", "I", "layoutmsg", "togglefit")
-- bind("Super+Ctrl", "F", "layoutmsg", "fit active")
-- bind("Super+Ctrl", "G", "layoutmsg", "fit visible")
-- bind("Super+Ctrl+Shift", "F", "layoutmsg", "fit all")
-- bind("Super+Ctrl", "H", "layoutmsg", "focus l")
-- bind("Super+Ctrl", "L", "layoutmsg", "focus r")
-- bind("Super+Ctrl", "K", "layoutmsg", "focus u")
-- bind("Super+Ctrl", "J", "layoutmsg", "focus d")
-- bind("Super+Ctrl+Shift", "H", "layoutmsg", "swapcol l")
-- bind("Super+Ctrl+Shift", "L", "layoutmsg", "swapcol r")
