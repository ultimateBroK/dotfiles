...
window_rules({
    ...
    -- conf: tag +fp_45x45 only for exact title ^(Library)$
    { match = { title = "^(Library)$" },              tag = "+fp_45x45" },
})
...
window_rules({
    ...
    { match = { xwayland = true }, immediate = true },
    { match = { xwayland = true }, no_blur = true },
    { match = { xwayland = true }, no_shadow = true },
    { match = { xwayland = true }, no_anim = true },
    -- Wine applications
    { match = { title = ".*\\.exe" },          immediate = true },
    { match = { class = "^(?i)wine.*" },       immediate = true },
    { match = { class = "^(?i)wine64.*" },     immediate = true },
    { match = { class = "^(?i)wine32.*" },     immediate = true },
    { match = { class = "^(?i)wine.*" },       no_blur = true },
    { match = { class = "^(?i)wine.*" },       no_shadow = true },
    { match = { class = "^(?i)wine.*" },       no_anim = true },
})
...
-- conf also: blur + ignore_alpha 0.6 on indicator.*
local ags_patterns = {
    { ns = "session[0-9]*", blur = true },
    { ns = "bar[0-9]*",     blur = true, alpha = 0.6 },
    { ns = "barcorner.*",   blur = true, alpha = 0.6 },
    { ns = "dock[0-9]*",    blur = true, alpha = 0.6 },
    { ns = "indicator.*",   blur = true, alpha = 0.6 },
    { ns = "overview[0-9]*", blur = true, alpha = 0.6 },
    { ns = "cheatsheet[0-9]*", blur = true, alpha = 0.6 },
    { ns = "sideright[0-9]*", blur = true, alpha = 0.6 },
    { ns = "sideleft[0-9]*", blur = true, alpha = 0.6 },
    { ns = "osk[0-9]*",     blur = true, alpha = 0.6 },
}
for _, p in ipairs(ags_patterns) do
    hl.layer_rule({ match = { namespace = p.ns }, blur = true })
    if p.alpha then
        hl.layer_rule({ match = { namespace = p.ns }, ignore_alpha = p.alpha })
    end
end
